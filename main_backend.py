...
while y <= 5:
    if sub_exist != True:
        ...
        cur2.execute("CREATE TABLE class(ClassID int, Name varchar(255), PRIMARY KEY (ClassID));")
        print("Class table created.")
        class_exist = True
        y = y + 1
    elif stu_exist != True:
        print("Student table not found, creating...")
        cur2.execute("CREATE TABLE student(StudentID int AUTO_INCREMENT, AdmissionNo varchar(255) UNIQUE, Name varchar(255), Gender varchar(255), PRIMARY KEY (StudentID));")
        print("Student table created.")
        stu_exist = True
        y = y + 1
    elif aca_exist != True:
        print("Academics table not found, creating...")
        cur2.execute("CREATE TABLE academics(AcademicID int AUTO_INCREMENT, StudentID int, Year int, ClassID int, SectionID int, RollNo int, SubjectID int, PRIMARY KEY (AcademicID), FOREIGN KEY (StudentID) REFERENCES student(StudentID), CONSTRAINT UC_StudentClassYear UNIQUE (StudentID, ClassID, Year));")
...
cur2.execute("ALTER TABLE sections AUTO_INCREMENT = 1;")
